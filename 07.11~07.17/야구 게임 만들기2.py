...
while len(answer_list) < 3 :
    answerValue       =    str(random.randint(1,9))
    # 정답 리스트 안에 값이 없는 경우만 넣어 주기
    if answerValue not in answer_list:
        answer_list.append(answerValue)
...
# 게임 안에서 계속 초기화 해줘야하는 변수 선언
    ...
    input_index         = 0
    # 입력 받은 값을 정답과 비교해주기
    for compareValue in inputValue_list:
        # 요소값이 정답 리스트에 있는 지 확인
        if compareValue in answer_list:
            # 인덱스 값도 맞다면 Strike
            if compareValue == answer_list[input_index]:
                Strike_count    +=  1
            # 인덱스 값이 다르면 Ball
            else :
                Ball_count      +=  1
    ...
    if try_count   >=  5    or   out_count   >=  2:
        ...
        if try_count   >=  5:
            ...
            print("아웃카운트 초과")
        print("아까비 졌네용..")
        print("정답은:",answer_list[0],answer_list[1],answer_list[2],"입니다.")
        flag = False
