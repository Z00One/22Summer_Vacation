...
gameCount = 0                                       # 게임횟수 카운트 변수 선언
while True:                                         # 게임 시작
    alphaCount = 0                                  # 알파벳 카운트 변수 선언
    gameCount += 1                                  # 게임 횟수 카운트
    
    inputAlpha =\
        input(str(gameCount) + "번째 시도, 아래 단어를 구성하는 알파벳 한 개 입력하세요\n" + blindWord + "\n" + "\n" )
    
    if inputAlpha in selectedWord:                  # 블라인드 제거
        for index in range(len(selectedWord)):      # 블라인드 제거할 인덱스 판별
            
            if selectedWord[index] == inputAlpha:
                alphaCount += 1                     # 알파벳이 단어 내에 있다면 카운트

                blindWordList[index] = inputAlpha   # 기존의 값으로 교체
    ...
    if alphaCount:                                  # 알파벳이 단어 내에 있다면 몇 글자인지 출력
        print("입력한 알파벳 단어 내 포함 : %d글자" %alphaCount)